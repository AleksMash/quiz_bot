...
def get_user_cache(user_id, data_id):
    return redis_db.get(f'{user_id}_{data_id}').decode('UTF-8')


def set_user_cache(user_id, data_id, value):
...
def start(event, vk_api, text=None):
    ...
    set_user_cache(event.user_id, CURRENT_KEYBOARD, KEYBOARD_MAIN)
    return Stages.START_POINT


def ask_question(event, vk_api):
    current_stage = int(get_user_cache(event.user_id, CURRENT_STAGE))
    if current_stage == Stages.TRY_AGAIN_YES_OR_NO:
        question = get_user_cache(event.user_id, QUESTION)
        msg_text = f'Повторяю вопрос\n\n:{question}'
    else:
        question = random.choice(questions)
        set_user_cache(event.user_id, QUESTION, question['q'])
        set_user_cache(event.user_id, RIGHT_ANSWER, question['a'])
        msg_text = question['q']
    vk_api.messages.send(
        user_id=event.user_id,
        message=msg_text,
    ...
    set_user_cache(event.user_id, CURRENT_KEYBOARD, KEYBOARD_GIVE_UP)
...
def process_answer(event, vk_api):
    ...
    set_user_cache(event.user_id, CURRENT_KEYBOARD, KEYBOARD_YES_NO)
    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
            ...
            if not current_stage:
                current_stage = start(event, vk_api, 'Выбери действие:')
            ...
            if not stage_callbacks:
                current_stage = start(event, vk_api, "Упс! Нет обработчиков для текущего шага."
            ...
            if not func:
                ...
                if not func:
                    ...
                    continue
            set_user_cache(event.user_id, CURRENT_STAGE, func(event, vk_api))
